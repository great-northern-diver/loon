...
with (minority,   ## Using the "with" function makes for easier reading
...
**Exercise:** Select the two histogram bars corresponding to the cities with the largest population.

Since this is spatial data it would be interesting to locate the cities on a map of Canada.  We again use the data from the `maps` package but now we will create an interactive map as well.
...
As can be seen, as can be seen, for most of these Canadian cities the visible minority population is less than 20\% of the whole population.
...
Some sense of the relative

```{r serialAxes, eval=FALSE}
# Back to the raw data and just rename variables
library(dplyr)
d_minority_star <- minority %>%
...
# Get axes sequence
library(PairViz)
set.seed(1564) # otherwise axes sequence differs
CorrelationMatrix <- cor(d_minority_star)
VarDistance <- dist(t(d_minority_star))
# use minus the distance to produce the greatest
# distinction between axes
o <- order_tsp(-VarDistance)

# west to east color mapping
library(scales)
west_east_cols <- (col_numeric(c("firebrick", "orange", "sienna",
                                 "darkgreen", "steelblue", "purple"),
                               domain = NULL))(minority$long)

s <- l_serialaxes(data=d_minority_star, linkingGroup = "minority",
                  sequence=names(d_minority_star)[o],
                  showGuides=FALSE,
                  linewidth=5,
                  scaling = "variable",
...
p_stars <- with(minority,
                l_plot(rank(long), rank(long),
                       ...
                       showScales = FALSE, showLabels=FALSE, size=3,
                       linkingGroup = "minority",
                       itemlabel=rownames(minority), showItemlabels=TRUE))

gstars <-l_glyph_add_serialaxes(p_stars , data=d_minority_star,
                                sequence = names(d_minority_star)[o],
                                scaling = 'variable',
                                showArea=TRUE)

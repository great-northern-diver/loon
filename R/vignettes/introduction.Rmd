...
```{r library loon, eval = TRUE, echo = TRUE, fig.align="center", fig.width = 6, fig.height = 4, out.width = "75%", warning=FALSE, message=FALSE}
library(loon)
```
...
- the `R` `help(package = "loon")` pages for `loon`,
- other vignettes,
...
- the loon website appearing from `l_help()`.
...
```{r first loon plot, eval = TRUE, echo = FALSE, fig.align="center", fig.width = 6, fig.height = 5, out.width = "75%", warning=FALSE, message=FALSE, tidy=FALSE}
...
We'll ignore the "inspector" for the moment and focus on the scatterplot.

--- 



##  The scatterplot window -- direct manipulation
...
At the top is a "World View" which shows a miniature version of the **whole** of the current plot, what is displayed there, and what is active.
...
```{r l_getColorList, eval = TRUE, tidy = TRUE}
l_getColorList()
```
...
## loon's `l_plot()` versus base graphics `plot()`
...
ranges <- apply(quakes, 2, FUN = function(x) diff(range(x)))

scaled_quakes <- as.data.frame(scale(quakes, scale = ranges))
...
Select the plot, type `r` to turn on the `rotation mode`.   Press the down arrow on the keyboard untill the darkest points (deepest quakes) are at the bottom of the display.   Press `r` again to turn off the rotation mode.
...
**NOTE** Many methods exist for visualizing higher dimensional data in `loon`.  For e.g. see `help("l_navgraph"), `help("l_pairs")`, or `help("l_serialaxes")`.  See also `l_help()`.
...
There are also numerous demos in loon whose source code might be examined for more examples of using `loon`.  Try these

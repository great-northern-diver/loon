...
loonGrob.l_serialaxes <- function(widget){
    ...
    scaling <- widget['scaling'] # which scaled method
    ...
    if(is.null(activeData) ) {
        ...
        scaledActiveData <- switch(scaling, 
                                   "variable" = t( ( t(activeData) - apply2min)/ 
                                                       (apply2max  - apply2min) ), 
                                   "observation" = (activeData - apply1min ) / (apply1max - apply1min ), 
                                   "data" = (activeData - min(dat))/ (max(dat) - min(dat)), 
                                   "none" = NULL)
    }
    
    activeSelected <- widget['selected'][active]
    activeColor <- as_hex6color( widget['color'][active])
    activeLinewidth <- widget['linewidth'][active]
    ...
    if(widget['axesLayout'] == "parallel"){
        ...
        if( is.null(scaledActiveData) ) parallelGrob <- NULL
        else {
            parallelGrob <- if(showArea){
                gTree(
                    children = gList(
                        do.call(
                            gList,
                        ...
                        if( any(activeSelected) ){
                            sel_Id <- which(activeSelected == TRUE )
                            gTree(
                                children =  
                                    do.call(
                                        gList,
                                        lapply(sel_Id, function(i){
                                            polygonGrob(
                                                x = unit( c(xaxis, rev(xaxis) ), "native"), 
                                                y = unit( c(scaledActiveData[i, ], rep(0, len.xaxis) ), "native"),
                                                gp = gpar(fill = sel_color, col = NA)
                                            )
                                        })
                                    ),
                                name = "parallelAxes"
                        ...
                        lapply(1:(len.xaxis + len.yaxis + 1), function(i) {
                            if(i == 1){
                                ...
                                if(showAxes) linesGrob(x = unit(rep(xaxis[i - 1],2 ), "native"), 
                                                       y =  unit(c(0, 1), "native"),
                                                       gp = gpar(col =  "white", lwd = 2)) else NULL
                            }else {
        ...
        if( is.null(scaledActiveData) ) radialGrob <- NULL
        else {
            radialGrob <- gTree(
                children = do.call(
                    gList,
            ...
            children = gList(
                if (showGuides){
                    gTree(children = gList(
                        rectGrob(gp = gpar(col = NA, fill = "#EBEBEB" )),
                        # circleGrob(gp = gpar(lwd = 2, col = "white", fill = NA), default.units = "native"),

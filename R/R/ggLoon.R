...
#'  
#'  p <- ggplot(data = SAheart, mapping = aes(x = age, y = chd)) +
#'    geom_smooth(method = "loess", colour = "steelblue") +
#'    geom_point(col="steelblue", size = 3, alpha = 0.4) +
#'    facet_wrap(~famhist)
#'  g <- ggLoon(p)
...
ggLoon <- function(ggplotObject, ... ){
  ...
  # ggplot_build  
  buildggplotObject <- ggplot2::ggplot_build(ggplotObject)
  # layout
  ...
  p <- lapply(1:panelNum, function(i){
    ...
    if(len_layers == 0) {
      ...
      if( length(pointsLayerId) != 0 ){
        ...
        for(j in 1:length(pointsLayerId)){
          pointData <- buildggplotObject$data[[pointsLayerId[j]]]
          data <- pointData[pointData$PANEL == i, ]
          x <- data$x
          y <- data$y
          ...
          if(j == 1){
            ...
            pointsX <- c(pointsX, x)
            pointsY <- c(pointsY, y)
            pointsColor <- c(pointsColor, color)
            pointsGlyph <- c(pointsGlyph, glyph)
            pointsSize <- c(pointsSize, size)
          }
        }
        
        loonPlot <- l_plot(parent = tt, x = pointsX, y = pointsY, size = pointsSize, 
                           title = title, color = pointsColor, glyph = pointsGlyph, 
                           xlabel = ggLabels$xlabel, ylabel = ggLabels$ylabel, 
                           showGuides = TRUE, showScales = TRUE, showLabels = TRUE)

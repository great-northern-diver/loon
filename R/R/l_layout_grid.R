...
l_layout_grid <- function(widget,
                          ...
                          parent = NULL, ...) {
    UseMethod("l_layout_grid", widget)
}

#' @export
l_layout_grid.loon <- function(widget,
                               by, linkingGroup,
                               inheritLayers = TRUE,
                               separate = FALSE,
                               byrow = TRUE,
    ...
    plots <- facet_grid_layout(plots = layouts$plots,
...
#' @export
l_layout_grid.l_serialaxes <- function(widget,
                                       by, linkingGroup,
                                       inheritLayers = TRUE,
                                       separate = FALSE,
                                       byrow = TRUE,
...
layout_grid_synchronizeSetting <- function(plots, child) {
    ...
    nrow <- max(layout_position[, "x"])
    ncol <- max(layout_position[, "y"])
    for(i in seq(nrow)) {
        row_id <- which(layout_position[, "x"] == i)
        # plots at the same row
        ps <- plots[row_id]
        zoomY <- min(vapply(ps, function(p) p['zoomY'], numeric(1L)))
        yrange <- c()
        lapply(ps,
               function(p) {
                   if(length(p['x']) > 0) {
                       ranges <- loonranges(p)
                       yrange <<- range(c(ranges$yrange, yrange), na.rm = TRUE)
                   }
               })
        if(length(yrange) == 0) yrange <- c(-0.01, 0.01)
    ...
    for(i in seq(ncol)) {
        col_id <- which(layout_position[, "y"] == i)
        # plots at the same column
        ps <- plots[col_id]
        zoomX <- min(vapply(ps, function(p) p['zoomX'], numeric(1L)))
        xrange <- c()
        lapply(ps,
               function(p) {
                   if(length(p['x']) > 0) {
                       ranges <- loonranges(p)
                       xrange <<- range(c(ranges$xrange, xrange), na.rm = TRUE)
                   }
               })
        if(length(xrange) == 0) xrange <- c(-0.01, 0.01)
...
#'
#' @export
l_getLocations.l_layout <- function(target) {
...
#'
#' @export
l_getPlots.l_layout <- function(target){
...
loonGrob_layoutType.l_layout <- function(target) "locations"
